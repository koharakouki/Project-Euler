# 下の20×20グリッドでは、対角線に沿った4つの数値が赤でマークされています。
# これらの数の積は、26×63×78×14 = 1788696です。
# 20×20グリッドで同じ方向（上、下、左、右、または対角線）に隣接する4つの数値の最大の積はいくつか？
...
table.each.with_index(0) do |row, i|
	row.each.with_index(0) do |number, j|
		if i <= 16 && j <= 16
			result_array.push(diagonally_right_down_array(table: table, row: i, column: j).inject(:*))
		end

		if i >= 3 && j <= 16
			result_array.push(diagonally_right_up_array(table: table, row: i, column: j).inject(:*))
		end
	end
end
